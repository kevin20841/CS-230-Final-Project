...
def generate_real_samples(path, names, n_samples, patch_shape):
    ...
    y1 = ones((n_samples, patch_shape, patch_shape, 1))
    y2 = ones((n_samples, patch_shape, patch_shape, 1))
    return X1t, X2t, y1, y2

# generate a batch of images, returns images and targets


def generate_fake_samples(g_model, dataset, patch_shape, n_samples):
...
def train(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, names,  path):
    print("Starting Training:")
    start = time.time()    
    # define properties of the training run
    n_epochs, n_batch, = 40, 10
    ...
    for i in range(n_steps):
        ...
        X_realA, X_realB, y_realA, y_realB = generate_real_samples(path, names, n_batch, n_patch)
        # generate a batch of fake samples
        X_fakeA, y_fakeA = generate_fake_samples(
            g_model_BtoA, X_realB, n_patch, n_batch)
        X_fakeB, y_fakeB = generate_fake_samples(
            g_model_AtoB, X_realA, n_patch, n_batch)
...
# train models
train(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, names, path)
